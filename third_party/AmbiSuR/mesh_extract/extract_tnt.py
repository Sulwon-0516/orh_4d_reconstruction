...
def post_process_mesh(mesh, cluster_to_keep=1, cluster_start=1):
    """
    ...
    n_cluster_start = np.sort(cluster_n_triangles.copy())[-cluster_start]
    n_cluster = max(n_cluster, 50) # filter meshes smaller than 50
    triangles_to_remove = (cluster_n_triangles[triangle_clusters] < n_cluster) | (cluster_n_triangles[triangle_clusters] > n_cluster_start)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
                 max_depth : float, voxel_size : float, sdf_trunc_scale : float, num_cluster: int, cluster_start: int, use_depth_filter : bool, max_height : float):
    with torch.no_grad():
        ...
        gaussians._opacity[-gaussians.get_xyz[:, 1] > max_height] = -1000.0
        ...
        if not skip_train:
            ...
            mesh = post_process_mesh(mesh, num_cluster, cluster_start)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cluster_start", default=1, type=int)
    parser.add_argument("--use_depth_filter", action="store_true")
    parser.add_argument("--max_height", default=200.0, type=float)
    ...
    print(args.num_cluster, args.cluster_start)
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.max_depth, args.voxel_size, args.sdf_trunc_scale, args.num_cluster, args.cluster_start, args.use_depth_filter, args.max_height)