            ...
            if nearest_cam is not None:
                ...
                if d_mask.sum() > 0:
                    ...
                    if use_virtul_cam is False:
                        with torch.no_grad():
                            '''debug'''
                            # Every 200th iteration this originally lifted the multi_view_sample_num
                            # cap and took EVERY pixel, purely so the patches could be reshaped to
                            # (H, W) and dumped as a PNG below.  lncc() then runs on that full-image
                            # tensor: N*49*4 bytes each, i.e. 20 MB at the 102400 cap but 661 MB per
                            # tensor at -r 1 (3.37 Mpx) -- which is the 614 MiB allocation in the
                            # -r 1 OOM.  Off by default; AMBISUR_FULL_NCC_DEBUG=1 restores it.
                            # The NCC loss is a mean, so the capped sample is an unbiased estimate of
                            # what the full-image pass computed.
                            _ncc_debug = os.environ.get("AMBISUR_FULL_NCC_DEBUG") == "1"
                            if _ncc_debug and iteration % 200 == 0:
                                sample_num = 1e9
